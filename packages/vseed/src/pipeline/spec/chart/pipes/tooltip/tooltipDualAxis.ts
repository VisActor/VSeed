...
import type { FoldInfo, Spec, SpecPipe, Tooltip, UnfoldInfo } from 'src/types'
import { findAllMeasures } from 'src/pipeline/utils'
import { getTooltipStyle } from './tooltipStyle'

export const tooltipPrimary: SpecPipe<Spec> = (spec, context) => {
  const result = { ...spec }
...
export const tooltipSecondary: SpecPipe<Spec> = (spec, context) => {
  const result = { ...spec }
  const { advancedVSeed, vseed } = context
  const { measures, datasetReshapeInfo, chartType, dimensions, encoding } = advancedVSeed
  const baseConfig = advancedVSeed.config[chartType] as { tooltip: Tooltip }
