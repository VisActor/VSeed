...
const Demo = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const tableName = 'supermarket'
      if (files.includes(tableName)) {
        await vquery.loadFile(tableName)
      } else {
        await vquery.registerFile(tableName, 'https://visactor.github.io/VSeed/dataset/supermarket.csv')
      }
