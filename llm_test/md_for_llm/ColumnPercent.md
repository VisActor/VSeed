# ColumnPercent 图表类型定义

`ColumnPercent` 组件用于渲染百分比柱状图。百分比柱状图适用于展示各类别占比关系的场景，Y轴以百分比形式展示数据占比。
...
export interface ColumnPercent {
  ...
  /**
   * 指标
   * @description 百分比柱状图的所有指标会自动合并为一个指标, 映射到Y轴, 存在多个指标时, 指标名称会与其余维度合并, 作为图例项展示。
   */
  measures?: Measures
  /**
   * 图表的背景颜色
