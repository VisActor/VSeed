...
async function getOpenAIJudgement(command: string, image1: Buffer, image2: Buffer): Promise<{ success: boolean; reason:string; }> {
    ...
    messages: [
      {
        role: 'user',
        content: [
          { type: 'text', text: `You are a helpful assistant for VSeed, a chart generation tool. Your task is to determine if a new chart configuration successfully fulfills a user's command, based on a "before" and "after" image of the chart. The user's command was: "${command}". Does the "after" image (the second image) correctly reflect this command compared to the "before" image (the first image)? Respond in JSON format with two keys: "success" (a boolean, true if the command was fulfilled) and "reason" (a string explaining your decision).` },
          {
            type: 'image_url',
            image_url: {
